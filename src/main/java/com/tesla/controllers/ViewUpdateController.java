...
public class ViewUpdateController extends HandleFields implements Initializable {
  ...
  public void updateDataset() {
    ...
    if (valid) {
        String productType = productTypeField.getText().trim(); // Trim any extra whitespace
        if (productType.isEmpty()) {
            AlertBox.displayAlert("Invalid Input", "Product type cannot be empty.");
            return;
        }
        Controller.updateDataset(dataset.getName(), productType, Integer.parseInt(numOfRecs.getText()), records1, records2);
        switchToPrimary();
    }
}
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        // Initialize the fields with dataset data
        ...
        // Initialize the productType field
        productTypeField.setText(dataset.getProductType()); // Assuming dataset has a getProductType() method
    }
